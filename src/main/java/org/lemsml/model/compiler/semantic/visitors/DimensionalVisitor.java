...
public class DimensionalVisitor extends BaseVisitor<Boolean, Throwable> {
	...
	public Boolean visit(Dimension dimension) throws Throwable {
		...
		javax.measure.Unit<?> old = this.lems.registerDimensionName(
				dimension.getName(), dimension.getUOMDimension());
		if (null != old) {
			warnMapOverwrite(dimension.getName(), old,
					dimension.getUOMDimension());
		}
		;
		return true;
	}
	...
	public Boolean visit(Unit unit) throws Throwable {
		...
		javax.measure.Unit<?> old = lems.registerUnitSymbol(unit.getSymbol(),
				uomUnit);
		if (null != old) {
	...
	private <K, V extends javax.measure.Unit<?>> void warnMapOverwrite(K key,
			V oldval, V newval) {
		StringBuilder sb = new StringBuilder();
